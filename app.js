...
function addMapplsMarker(l,opts={}){
  const marker=new mappls.Marker({
    ...
    offset:[0,0],
    popupOptions:opts.popupHtml?(opts.popupOptions||{autoClose:true,maxWidth:340,className:'lis-location-popup',offset:{bottom:[0,-18]}}):undefined,
...
function renderMarkers(filterType=null){
  ...
  locations.filter(l=>!filterType||l.type===filterType).forEach(l=>{
    try{
      const popup=`<div class="location-popup" style="width:320px;max-width:320px;min-width:320px;box-sizing:border-box;">
        <b>${esc(l.name)}</b>
        <span class="location-popup-type">${esc(l.type)}${l.code?' · '+esc(l.code):''}</span>
        ${l.capacity?`<small>Capacity: ${esc(l.capacity)} MT</small>`:''}
        ${l.railhead?`<small>Rail Head / Cluster: ${esc(l.railhead)}</small>`:''}
        <small>${Number(l.lat).toFixed(6)}, ${Number(l.lng).toFixed(6)}</small>
...
async function goToLocationOnMap(id){
  ...
  try{
    ...
    window.__locationFocusMarker=addMapplsMarker(loc,{
      ...
      width:28,
      height:28,
      popupOptions:{autoClose:true,maxWidth:340,className:'lis-location-popup',offset:{bottom:[0,-18]}},
      popupHtml:`<div class="location-popup" style="width:320px;max-width:320px;min-width:320px;box-sizing:border-box;">
        <b>${esc(loc.name)}</b><br>
...
function routeLabelPosition(latlngs){
  if(!latlngs||latlngs.length<2)return null;
  // Use the midpoint of the returned road geometry; this keeps the label
  // on the route rather than at an arbitrary map corner.
...
function drawRouteDistanceLabels(valid){
  ...
  (valid||[]).forEach(({index,latlngs})=>{
    ...
    const pos=routeLabelPosition(latlngs);
    if(!r||!pos)return;
...
function drawAllMapplsRoutes(routes,selectedIndex=0){
  ...
  (routes||[]).forEach((r,i)=>{
    ...
    const layer=new mappls.Polyline({
      ...
      strokeWeight:selected?8:6,
      fitbounds:false,
      lineCap:'round',
      lineJoin:'round',
      zIndex:selected?40:20+i,
      popupHtml:`${i===0?'Mappls Primary Route':'Mappls Alternative '+i} — ${routeDistanceKm(r)==null?'—':routeDistanceKm(r).toFixed(1)+' KM'}`
    });
