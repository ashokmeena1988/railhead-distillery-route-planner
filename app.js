...
function drawAllMapplsRoutes(routes,selectedIndex=0){
  ...
  (routes||[]).forEach((r,i)=>{
    ...
    // Mappls Web Maps JS expects the Polyline coordinate collection in `paths`.
    // Using `path` silently produces no visible route in the vector SDK.
    const paths=ll.map(([lat,lng])=>({lat:Number(lat),lng:Number(lng)}));
    const selected=i===selectedIndex;
    const color=routeColor(i);

    // White casing gives the selected route the clean, Google-Maps-like separation
    // from the base map and from overlapping alternatives.
    // Mappls Web Maps JS documents `path` as the primary Polyline property.
    // Keep a `paths` retry for SDK builds that expose the alternate property.
    const makePolyline=(opts)=>{
      try{return new mappls.Polyline(opts)}catch(e){
        return new mappls.Polyline({...opts,paths:opts.path});
      }
    };
    const casing=makePolyline({
      map, path:paths,
      strokeColor:'#ffffff', strokeOpacity:0.96,
      strokeWeight:selected?12:8, fitbounds:false, zIndex:selected?45:10+i
    });
    const layer=makePolyline({
      map, path:paths,
...
function renderDepotDistilleryTable(){
  ...
  h+='</tbody></table>';
  el.innerHTML=`<div class="report-table-scroll" role="region" aria-label="Depot-wise distillery report table">${h}</div>`;
}
