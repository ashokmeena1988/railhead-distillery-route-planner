...
function clearRoute(){if(routeLayer){removeMapplsLayer(routeLayer);routeLayer=null}clearRouteOverlays?.();routeAlternatives=[];selectedRouteIndex=0;lastRoute=null;$('km').textContent='—';$('time').textContent='—';$('legs').textContent='—';$('routeSequence').innerHTML='<div class="empty">No route calculated.</div>';$('routeDetails').innerHTML='<div class="empty">Calculate a route to see details.</div>';if($('routeSummaryLegs'))$('routeSummaryLegs').innerHTML='<div class="empty">No route calculated.</div>';if($('routeAlternativeSummary'))$('routeAlternativeSummary').innerHTML='<div class="empty">No alternative routes calculated.</div>';if($('routeAltCount'))$('routeAltCount').textContent='—';$('mapSummary').classList.remove('show');$('mapRouteDetailsOverlay')?.classList.remove('show');$('routeStatus').className='status';$('routeStatus').textContent='Ready.'}
...
function updateMapRouteOverlay(){
  ...
  box.innerHTML=`
    <div class="map-route-overlay-head"><div><b>${idx===0?'Primary Route':'Alternative '+idx}</b><span>Selected route</span></div><button class="map-route-overlay-close" type="button" aria-label="Close route details">×</button></div>
    <div class="map-route-overlay-metrics"><div><strong>${km==null?'—':km.toFixed(1)+' KM'}</strong><span>Road distance</span></div><div><strong>${esc(tm)}</strong><span>Travel time</span></div><div><strong>${points.length>1?points.length-1:'—'}</strong><span>Route legs</span></div></div>
...
function renderSelectedRouteDetails(route,index){
  ...
  const legs=apiLegs.length?apiLegs:[{
    ...
    km:routeDistanceKm(route),time:routeTimeText(route)
  }];

  let h=`<div class="hint selected-route-note"><b>${index===0?'Primary Route':'Alternative '+index}</b> selected on the map. Distance and time below are from the selected Mappls route.</div>`;
...
function selectMapplsRoute(index){
  ...
  // Rebuild all returned routes, then add a dedicated strong highlight to the
  // selected one. This avoids SDK-specific style mutation APIs.
  if(window.__allRouteLayers)window.__allRouteLayers.forEach(removeMapplsLayer);
  window.__allRouteLayers=[];
...
function renderAlternativeRoutes(routes){
  ...
  const routeNote=routeAlternatives.length===1
    ...
    :`<div class="hint" style="margin:6px 0 9px;padding:8px;border-radius:8px;border:1px solid #d9e7e1;background:#f7fbf9">Mappls returned ${routeAlternatives.length} routes. Click any route to highlight it on the map and update the details.</div>`;
