...
async function goToLocationOnMap(id){
  ...
  try{
    ...
    const focus=()=>{
      ...
      // A tiny two-point bounds is used as a fallback because some Mappls SDK
      // viewport transitions can ignore a direct setCenter immediately after a tab switch.
      try{mapplsFitBounds([{lat:lat-0.00035,lng:lng-0.00035},{lat:lat+0.00035,lng:lng+0.00035}],40);}catch(_){ }
      try{map.setCenter({lat,lng});map.setZoom(15);}catch(_){ }
    };
    focus();
    setTimeout(focus,180);
    setTimeout(focus,650);
    setTimeout(()=>{try{window.__locationFocusMarker?.openPopup?.();}catch(_){ }},700);
...
function renderSelectedRouteDetails(route,index){
  ...
  const apiLegs=routeApiLegDetails(route,points);
  const legs=apiLegs.length?apiLegs:[{
    from:points[0],to:points[points.length-1],
    km:Number.isFinite(Number(route?.distance))?Number(route.distance)/1000:null,
    time:Number.isFinite(Number(route?.duration))?formatDurationSeconds(Number(route.duration)):'—'
  }];

  let h=`<div class="hint" style="margin-bottom:8px;padding:8px;border-radius:8px;border:1px solid #d9e7e1;background:#f7fbf9"><b>${index===0?'Primary Route':'Alternative '+index}</b> selected on the map.</div>`;
  h+='<table class="table"><thead><tr><th>#</th><th>From</th><th>To</th><th>Road Distance</th><th>Travel Time</th></tr></thead><tbody>';
  legs.forEach((x,i)=>{
    h+=`<tr><td>${i+1}</td><td>${esc(x.from?.name||'—')}</td><td>${esc(x.to?.name||'—')}</td><td><b>${x.km==null?'—':x.km.toFixed(1)+' KM'}</b></td><td>${esc(x.time||'—')}</td></tr>`;
  });
  h+='</tbody></table>';
...
function selectMapplsRoute(index){
  ...
  // Rebuild every returned route, then add a strong highlight for the selected route.
  if(window.__allRouteLayers)window.__allRouteLayers.forEach(removeMapplsLayer);
  window.__allRouteLayers=[];
  ...
  mapplsFitBounds(selected.latlngs,60);
  ...
  renderRouteMetrics(Number.isFinite(km)?km:null,tm,`Mappls ${index===0?'primary':'alternative '+index} route`);
  renderSelectedRouteDetails(r,index);
  $('routeStatus').className='status ok';
  $('routeStatus').textContent=`✓ Mappls ${index===0?'Primary':'Alternative '+index} route selected — ${Number.isFinite(km)?km.toFixed(1)+' KM':'—'} · ${tm}`;
  if($('kpiDistance')&&Number.isFinite(km))$('kpiDistance').textContent=km.toFixed(1)+' km';
...
function renderAlternativeRoutes(routes){
  ...
  if(!box)return;
  if(!routeAlternatives.length){box.innerHTML='<div class="empty">No routes returned by Mappls.</div>';return;}
  ...
  const routeNote=routeAlternatives.length===1
    ...
    :`<div class="hint" style="margin:6px 0 9px;padding:8px;border-radius:8px;border:1px solid #d9e7e1;background:#f7fbf9">Mappls returned ${routeAlternatives.length} routes. Click a route below to highlight it and update the route details.</div>`;
