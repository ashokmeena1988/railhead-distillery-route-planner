...
window.__routeDistanceLabels=[];
window.__locationInfoWindow=null;
...
function mapplsFitBounds(points,padding=60){
  ...
  pts.forEach(([lat,lng])=>{minLat=Math.min(minLat,lat);maxLat=Math.max(maxLat,lat);minLng=Math.min(minLng,lng);maxLng=Math.max(maxLng,lng)});
  const center={lat:(minLat+maxLat)/2,lng:(minLng+maxLng)/2};
  const latSpan=Math.max(0.05,maxLat-minLat);
  const lngSpan=Math.max(0.05,maxLng-minLng);
  const fallbackZoom=()=>Math.max(4,Math.min(12,Math.min(12,Math.max(5,Math.min(Math.log2(300/Math.max(lngSpan,0.1)),Math.log2(150/Math.max(latSpan,0.1)))))));
  try{
    new mappls.fitBounds({
      map:map,
      cType:0,
      bounds:[[minLng,minLat],[maxLng,maxLat]],
      options:{padding,duration:650}
    });
    setTimeout(()=>{try{map.setCenter(center);if(typeof map.setZoom==='function')map.setZoom(fallbackZoom())}catch(_){}},720);
  }catch(e){
    try{map.setCenter(center);if(typeof map.setZoom==='function')map.setZoom(fallbackZoom())}catch(_){}
  }
}
...
function closeLocationInfoWindow(){
  if(!window.__locationInfoWindow||!map)return;
  try{mappls.remove({map:map,layer:window.__locationInfoWindow});}catch(_){
    try{window.__locationInfoWindow.remove?.();}catch(__){}
  }
  window.__locationInfoWindow=null;
}

function buildLocationInfoHtml(l){
  return `<div class="lis-info-card">
    <div class="lis-info-title">${esc(l.name)}</div>
    <div class="lis-info-type">${esc(l.type)}${l.code?' · '+esc(l.code):''}</div>
    ${l.capacity?`<div class="lis-info-row"><b>Capacity</b><span>${esc(l.capacity)} MT</span></div>`:''}
    ${l.railhead?`<div class="lis-info-row"><b>Rail Head / Cluster</b><span>${esc(l.railhead)}</span></div>`:''}
    <div class="lis-info-coords">${Number(l.lat).toFixed(6)}, ${Number(l.lng).toFixed(6)}</div>
    <div class="lis-info-actions">
...
function openLocationInfoWindow(l){
  if(!map||!l)return;
  closeLocationInfoWindow();
  try{
    window.__locationInfoWindow=new mappls.InfoWindow({
      map,
      position:{lat:Number(l.lat),lng:Number(l.lng)},
      content:buildLocationInfoHtml(l),
      class:'lis-info-window',
      offset:[0,-18],
      maxWidth:340,
      closeButton:true
    });
...
function addMapplsMarker(l,opts={}){
  ...
  if(opts.locationInfo){
    try{marker.addListener('click',()=>openLocationInfoWindow(opts.locationInfo));}catch(e){console.warn('Location marker click binding failed',e)}
  }else if(opts.popupHtml){
    // Keep popup behavior only for route endpoints and other non-location markers.
    try{
      marker.setPopup?.({content:opts.popupHtml});
    }catch(_){}
  }
  return marker;
}
...
function renderMarkers(filterType=null){
  ...
  locations.filter(l=>!filterType||l.type===filterType).forEach(l=>{
    try{
      markers.push(addMapplsMarker(l,{locationInfo:l}));
...
function clearRoute(){
  try{closeLocationInfoWindow();}catch(e){}
...
async function goToLocationOnMap(id){
  ...
  try{
    ...
    setTimeout(()=>{
      try{openLocationInfoWindow(loc);}catch(_){}
...
function renderRouteEndpointMarkers(points){
  ...
  (points||[]).forEach((p,i)=>{
    ...
    const marker=addMapplsMarker(p,{
      ...
      width:30,
      height:30,
      locationInfo:{...p, type:i===0?'Origin':i===points.length-1?'Destination':`Via ${i}`}
    });
...
function clearRouteOverlays(){
  clearRouteEndpointMarkersOnly();
  if(window.__routeDistanceLabels){
    window.__routeDistanceLabels.forEach(removeMapplsLayer);
    window.__routeDistanceLabels=[];
  }
...
function routeColor(index){
  return ['#078b4e','#e67e22','#6c4cff','#0086c9','#d33f8d','#7b61a8'][index%6];
}
function routeLabelPosition(latlngs,fraction=.5){
  if(!latlngs||latlngs.length<2)return null;
  // Keep the badge on the returned road geometry. The fraction is varied for
  // alternatives so the three badges do not sit directly on top of each other.
  const f=Math.max(.08,Math.min(.92,Number(fraction)||.5));
  const mid=(latlngs.length-1)*f;
  const i=Math.floor(mid), t=mid-i;
  const a=latlngs[i],b=latlngs[Math.min(i+1,latlngs.length-1)];
  return [a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t];
}
function drawRouteDistanceLabels(valid){
  if(window.__routeDistanceLabels)window.__routeDistanceLabels.forEach(removeMapplsLayer);
  window.__routeDistanceLabels=[];
  (valid||[]).forEach(({index,latlngs})=>{
    const r=routeAlternatives[index];
    const fractions=[.50,.34,.66,.25,.75];
    const pos=routeLabelPosition(latlngs,fractions[index]??.5);
    if(!r||!pos)return;
    const km=routeDistanceKm(r),tm=routeTimeText(r);
    const selected=index===selectedRouteIndex;
    // Keep the map clean: Google-Maps-style compact route badges, not large popups.
    const title=index===0?'Primary Route':'Alternative '+index;
    const html=`<div class="route-distance-pin ${selected?'selected':''}" style="--route-pin:${routeColor(index)}"><b>${esc(title)}</b><span>${km==null?'—':km.toFixed(1)+' km'} · ${esc(tm)}</span></div>`;
    const marker=new mappls.Marker({
      map,
      position:{lat:Number(pos[0]),lng:Number(pos[1])},
      html,
      width:selected?156:118,
      height:selected?44:36,
      offset:[0,0],
      draggable:false
    });
    window.__routeDistanceLabels.push(marker);
  });
}
function drawAllMapplsRoutes(routes,selectedIndex=0){
  ...
  (routes||[]).forEach((r,i)=>{
    ...
    // Mappls Web JS documents `path` as the Polyline coordinate property.
    // Keep one canonical rendering path so route drawing cannot silently diverge
    // between SDK implementations.
    const path=ll.map(([lat,lng])=>({lat:Number(lat),lng:Number(lng)}));
    const selected=i===selectedIndex;
    const color=routeColor(i);

    // White casing separates alternatives from the basemap and from each other.
    const casing=new mappls.Polyline({
      map, path,
      strokeColor:'#ffffff', strokeOpacity:0.96,
      strokeWeight:selected?12:8, fitbounds:false, zIndex:selected?45:10+i
    });
    const layer=new mappls.Polyline({
      map, path,
      strokeColor:color, strokeOpacity:selected?1:0.72,
      strokeWeight:selected?7:4, fitbounds:false, zIndex:selected?50:20+i
    });
    try{
      layer.addListener('click',()=>selectMapplsRoute(i));
      casing.addListener('click',()=>selectMapplsRoute(i));
    }catch(e){}
    window.__allRouteLayers.push(casing,layer);
    valid.push({index:i,latlngs:ll,layer,casing});
  });
  drawRouteDistanceLabels(valid);
  return valid;
}
...
function selectMapplsRoute(index){
  ...
  // Keep every returned route visible. Selection changes styling/details only;
  // do not refit to one alternative because that can push other alternatives off-screen.
  ...
  toast(`Mappls ${index===0?'primary':`alternative ${index}`} route selected and highlighted.`);
}
...
function renderAlternativeRoutes(routes){
  ...
  const routeNote=routeAlternatives.length===1
    ...
    :`<div class="hint" style="margin:6px 0 9px;padding:8px;border-radius:8px;border:1px solid #d9e7e1;background:#f7fbf9">Mappls returned ${routeAlternatives.length} routes. All returned routes stay visible on the map. Click a route to highlight it and update the details.</div>`;
...
async function renderMapplsRoute(points,profile,resource){
  ...
  const allPoints=plotted.flatMap(x=>x.latlngs||[]);
  if(allPoints.length>=2)mapplsFitBounds(allPoints,70);
  selectMapplsRoute(0);
...
async function calculateRoute(){
  ...
  try{
    // Use the routing API's own distance/time as the single source of truth.
    // The previous implementation made a second matrix call, which could
    // produce a different number from the route actually drawn on the map.
    const plotted=await renderMapplsRoute(points,profile,resource);
    const routes=plotted.routes||[];
    const primary=routes[0];
    selectedRouteIndex=0;
    const km=routeDistanceKm(primary);
    const time=routeTimeText(primary);
    const legs=routeLegRows(primary,points);
    lastRoute={km,time,legs,coords:points.map(p=>[p.lng,p.lat]),geometry:routeGeometryToLatLngs(primary)};
    renderRouteMetrics(km,time,'Mappls Primary Route');
    renderSelectedRouteDetails(primary,0);
    updateMapRouteOverlay();
    saveRouteHistory(points,km,time);
    showSequence();renderLegMetrics(legs);
    $('routeStatus').className='status ok';$('routeStatus').textContent=`✓ Mappls route calculated — ${routes.length} route${routes.length===1?'':'s'} returned · ${km==null?'—':km.toFixed(1)+' KM'} · ${time||'time unavailable'}`;
    if($('kpiDistance')&&km!=null)$('kpiDistance').textContent=km.toFixed(1)+' km';
...
function renderRankTable(elId, rows, label){
  ...
  h+='</tbody></table>';
  el.innerHTML=`<div class="report-table-scroll" role="region" aria-label="${esc(label)} report table">${h}</div>`;
}
...
let depotReportBusy=false;
let depotReportRequestId=0;
...
function renderDepotDistilleryTable(){
  ...
  h+='</tbody></table>';
  // This report contains one row per distillery (normally 14). Keep it in the
  // normal Reports document flow so the page scrolls naturally and no nested
  // viewport can hide the lower rows. The table header remains sticky via CSS.
  el.innerHTML=h;
}

async function generateDepotDistilleryReport(){
  if(depotReportBusy)return;
  const requestId=++depotReportRequestId;
  const depot=getLoc($('reportDepot')?.value);
  if(!depot){ resetDepotReportControls('Select a depot / godown first.'); return toast('Select a depot / godown first.',true); }
  const profile=$('depotReportProfile')?.value || 'trucking';
  const dir=$('depotReportDirection')?.value || 'depot_to_distillery';
  const distilleries=locations.filter(x=>x.type==='Distillery');
  if(!distilleries.length)return toast('No distilleries are configured.',true);

  // This report is a data/matrix operation. It must not depend on the Map UI
  // being initialized or visible.
  const resource='distance_matrix';
  depotReportBusy=true;
  setDepotReportBusy(true);
  resetDepotReportResultOnly();
  const status=$('depotReportStatus');
  status.className='status';
  status.textContent=`Calculating ${distilleries.length} distilleries for ${depot.name}…`;
  try{
    let result;
    ...
    if(requestId!==depotReportRequestId)return;
    const rows=distilleries.map((d,i)=>{
      let meters=null,seconds=null,from,to;
      if(dir==='depot_to_distillery'){
    ...
    status.textContent=`Report ready: ${depot.name} · ${dir==='depot_to_distillery'?'Depot → Distillery':'Distillery → Depot'} · ${profile==='trucking'?'Truck / HCV':'Car / LMV'}.`;
    $('exportDepotDistilleryReport').disabled=false;
    toast('Depot-wise distillery report generated.');
  }catch(e){
    if(requestId!==depotReportRequestId)return;
    console.error('Depot report failed',e);
    status.className='status err';
    status.textContent='Report failed: '+e.message;
    $('depotDistilleryReportTable').innerHTML=`<div class="status err" style="margin:12px"><b>Depot-wise report failed</b><br>${esc(e.message)}</div>`;
    toast('Depot-wise report failed: '+e.message,true);
  }finally{
    if(requestId===depotReportRequestId){depotReportBusy=false;setDepotReportBusy(false);}
  }
}

function resetDepotReportResultOnly(){
  lastDepotDistilleryReport=null;
  depotReportSort='asc';
  ['depotReportDistilleryCount','depotReportNearestKm','depotReportFarthestKm','depotReportAverageKm'].forEach(id=>{if($(id))$(id).textContent='—'});
  const exp=$('exportDepotDistilleryReport'); if(exp)exp.disabled=true;
  const table=$('depotDistilleryReportTable');
  if(table)table.innerHTML='<div class="empty">Click Generate Depot-wise Report to calculate the selected depot.</div>';
  document.querySelectorAll('#depotReportNearest,#depotReportFarthest').forEach(b=>b.classList.remove('active'));
}

function setDepotReportBusy(busy){
  const ids=['runDepotDistilleryReport','depotReportNearest','depotReportFarthest','exportDepotDistilleryReport','reportDepot','depotReportDirection','depotReportProfile'];
  ids.forEach(id=>{const el=$(id);if(el)el.disabled=busy || (id==='exportDepotDistilleryReport' && !lastDepotDistilleryReport);});
  const run=$('runDepotDistilleryReport');
  if(run)run.innerHTML=busy?'⏳ Calculating…':'▶ Generate Depot-wise Report';
}
...
function resetDepotReportControls(message='Select a depot and generate the distillery report.') {
  depotReportRequestId++;
  depotReportBusy=false;
  lastDepotDistilleryReport=null;
  depotReportSort='asc';
  const status=$('depotReportStatus');
  if(status){status.className='status';status.textContent=message;}
  ['depotReportDistilleryCount','depotReportNearestKm','depotReportFarthestKm','depotReportAverageKm'].forEach(id=>{if($(id))$(id).textContent='—'});
  const exp=$('exportDepotDistilleryReport'); if(exp)exp.disabled=true;
  const table=$('depotDistilleryReportTable');
  if(table)table.innerHTML='<div class="empty">Select a depot, choose direction and vehicle, then click Generate Depot-wise Report.</div>';
  document.querySelectorAll('#depotReportNearest,#depotReportFarthest').forEach(b=>b.classList.remove('active'));
  setDepotReportBusy(false);
}
$('runDepotDistilleryReport').addEventListener('click',generateDepotDistilleryReport);
$('exportDepotDistilleryReport').addEventListener('click',exportDepotDistilleryReport);
$('reportDepot').addEventListener('change',()=>resetDepotReportControls('Depot changed. Click Generate Depot-wise Report to calculate the new distances.'));
$('depotReportDirection').addEventListener('change',()=>resetDepotReportControls('Direction changed. Click Generate Depot-wise Report to recalculate the report.'));
$('depotReportProfile').addEventListener('change',()=>resetDepotReportControls('Vehicle changed. Click Generate Depot-wise Report to recalculate the report.'));
$('depotReportNearest').onclick=()=>{
  depotReportSort='asc';
  document.querySelectorAll('#depotReportNearest,#depotReportFarthest').forEach(b=>b.classList.remove('active'));
  $('depotReportNearest')?.classList.add('active');
  if(lastDepotDistilleryReport){renderDepotDistilleryTable(); $('depotReportStatus').textContent='Report ready · Sorted nearest to farthest.';}
  else toast('Generate the depot-wise report first.',true);
};
$('depotReportFarthest').onclick=()=>{
  depotReportSort='desc';
  document.querySelectorAll('#depotReportNearest,#depotReportFarthest').forEach(b=>b.classList.remove('active'));
  $('depotReportFarthest')?.classList.add('active');
  if(lastDepotDistilleryReport){renderDepotDistilleryTable(); $('depotReportStatus').textContent='Report ready · Sorted farthest to nearest.';}
  else toast('Generate the depot-wise report first.',true);
};
...
$('showAll').onclick=()=>{if(!map)return toast('Please open Map View first.',true);renderMarkers();fitAll();toast('All locations shown and map fitted to the operating network.');};
...
async function showRoutePlannerView(){
  ...
  if(!ws)return;
  if(!(await initMap()))return;
  ws.classList.remove('mapfocus','fullmode','dashboard-mapview');
...
async function showMapView(options={}){
  ...
  // Dashboard / Map View keeps the Route Planner visible beside the map.
  // Do not use mapfocus here: mapfocus intentionally hides the right column.
  ws.classList.remove('routefocus','fullmode','mapfocus');
  ws.classList.add('dashboard-mapview');
  document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));
  ...
  setTimeout(()=>{
    try{
      ...
      if(!preserveViewport){
        if(lastRoute?.geometry?.length)mapplsFitBounds(lastRoute.geometry,80);
        else if($('origin').value&&$('destination').value){
          const a=getLoc($('origin').value),b=getLoc($('destination').value);
          if(a&&b)mapplsFitBounds([a,b],80); else fitAll();
        }else fitAll();
      }
...
for(const t of document.querySelectorAll('.tab'))t.onclick=async()=>{
  ...
  if(t.dataset.tab==='reports'){fillDistilleryReportSelect();fillDepotReportSelect();resetDepotReportControls('Select a depot, choose direction and vehicle, then click Generate Depot-wise Report.');setTimeout(()=>document.querySelector('.main')?.scrollTo({top:0,behavior:'instant'}),50)}
  document.querySelectorAll('.topnav button').forEach(x=>x.classList.remove('active'));
...
(function boot(){
  fillSelects(); renderRouteHistory(); refreshKpis(); fillDistilleryReportSelect(); fillDepotReportSelect();
