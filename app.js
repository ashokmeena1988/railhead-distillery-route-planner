...
let routeAlternatives=[], directionPanelCollapsed=false;
...
function clearRoute(){if(routeLayer){removeMapplsLayer(routeLayer);routeLayer=null}clearRouteOverlays?.();routeAlternatives=[];lastRoute=null;$('km').textContent='—';$('time').textContent='—';$('legs').textContent='—';$('routeSequence').innerHTML='<div class="empty">No route calculated.</div>';$('routeDetails').innerHTML='<div class="empty">Calculate a route to see details.</div>';if($('routeSummaryLegs'))$('routeSummaryLegs').innerHTML='<div class="empty">No route calculated.</div>';if($('routeAlternativeSummary'))$('routeAlternativeSummary').innerHTML='<div class="empty">No alternative routes calculated.</div>';if($('routeAltCount'))$('routeAltCount').textContent='—';$('mapSummary').classList.remove('show');$('routeStatus').className='status';$('routeStatus').textContent='Ready.'}
...
async function goToLocationOnMap(id){
  ...
  try{
    const ok=await showMapView();
    if(!ok){toast('Map is not initialized.',true);return false;}

    if(window.__locationFocusMarker)removeMapplsLayer(window.__locationFocusMarker);

    const html='<div class="location-focus-pin"></div>';
    window.__locationFocusMarker=addMapplsMarker(loc,{
      ...
      popupHtml:`<div style="font-family:Arial;padding:10px;min-width:230px">
        ...
        <br><small>${Number(loc.lat).toFixed(6)}, ${Number(loc.lng).toFixed(6)}</small>
      </div>`
    });
...
function decodePolyline(str){
  ...
  while(index<str.length){
    ...
    do{b=str.charCodeAt(index++)-63;result|=(b&31)<<shift;shift+=5}while(b>=32&&index<str.length);
...
function geometryToLatLngs(geometry){
  if(!geometry)return [];
  if(Array.isArray(geometry))return geometry.map(p=>Array.isArray(p)?[Number(p[1]),Number(p[0])]:[Number(p.lat),Number(p.lng)]).filter(p=>p.every(Number.isFinite));
  if(typeof geometry==='object' && Array.isArray(geometry.coordinates))return geometry.coordinates.map(p=>[Number(p[1]),Number(p[0])]);
  if(typeof geometry==='string'){
    try{const j=JSON.parse(geometry);if(j?.coordinates)return geometryToLatLngs(j)}catch(e){}
    return decodePolyline(geometry);
  }
  return [];
}
function renderRouteEndpointMarkers(points){
  if(window.__routeEndpointMarkers){window.__routeEndpointMarkers.forEach(m=>{try{map.removeLayer(m)}catch(e){}})}
  window.__routeEndpointMarkers=[];
  points.forEach((p,i)=>{
    const cls=i===0?'route-start':i===points.length-1?'route-end':'route-via';
    const label=i===0?'A':i===points.length-1?'B':String(i);
    const icon=L.divIcon({className:'route-endpoint-wrap',html:`<div class="route-endpoint-pin ${cls}">${label}</div>`,iconSize:[30,30],iconAnchor:[15,15]});
    const m=L.marker([Number(p.lat),Number(p.lng)],{icon,zIndexOffset:2500}).addTo(map);
    m.bindTooltip(`<b>${i===0?'ORIGIN':i===points.length-1?'DESTINATION':'VIA '+i}</b><br>${esc(p.name)}`,{direction:'top',offset:[0,-14]});
    window.__routeEndpointMarkers.push(m);
  });
}
function renderAlternativeRoutes(routes){
  ...
  if(!box)return;
  if(!routeAlternatives.length){box.innerHTML='<div class="empty">No alternative routes returned by Mappls.</div>';return;}
  box.innerHTML=routeAlternatives.map((r,i)=>{
    const km=Number(r.distance)/1000;
    const sec=Number(r.duration);
    const tm=Number.isFinite(sec)?formatDurationSeconds(sec):'—';
    return `<div class="history-item" style="cursor:pointer" data-alt-route="${i}"><div><b>${i===0?'Primary Route':'Alternative '+i}</b><div class="history-meta">${Number.isFinite(km)?km.toFixed(1)+' KM':'—'} · ${esc(tm||'—')}</div></div><span class="badge">${i===0?'SELECTED':'VIEW'}</span></div>`;
  }).join('');
  box.querySelectorAll('[data-alt-route]').forEach(el=>el.onclick=()=>{
    const i=Number(el.dataset.altRoute),r=routeAlternatives[i],latlngs=geometryToLatLngs(r?.geometry);
    if(!latlngs.length)return toast('This alternative route has no map geometry.',true);
    if(window.__selectedAltLayer){try{map.removeLayer(window.__selectedAltLayer)}catch(e){}}
    window.__selectedAltLayer=L.polyline(latlngs,{weight:5,opacity:.65,dashArray:'10 8',lineCap:'round'}).addTo(map);
    map.fitBounds(L.latLngBounds(latlngs),{padding:[60,60]});
    toast(`${i===0?'Primary':'Alternative '+i} route shown on map.`);
  });
}
...
function drawAllMapplsRoutes(routes){
  ...
  (routes||[]).forEach((r,i)=>{
    ...
    const layer=new mappls.Polyline({
      map,
      path,
      strokeColor:i===0?'#0b7a55':'#2f80ed',
      strokeOpacity:i===0?0.95:0.65,
      strokeWeight:i===0?8:5,
      fitbounds:false,
...
function selectMapplsRoute(index){
  ...
  // Rebuild route overlays to avoid relying on SDK-specific setStyle APIs.
  if(window.__allRouteLayers)window.__allRouteLayers.forEach(removeMapplsLayer);
  window.__allRouteLayers=[];

  const valid=drawAllMapplsRoutes(routeAlternatives);
  const selected=valid.find(x=>x.index===index);
  if(selected)mapplsFitBounds(selected.latlngs,60);
  ...
  const km=Number(r.distance)/1000;
  const tm=Number.isFinite(Number(r.duration))?formatDurationSeconds(Number(r.duration)):'—';
  $('routeStatus').className='status ok';
  $('routeStatus').textContent=`✓ Mappls Route ${index+1} selected — ${Number.isFinite(km)?km.toFixed(1)+' KM':'—'} · ${tm}`;
  toast(`Mappls ${index===0?'primary':`alternative ${index}`} route selected.`);
}
...
function renderAlternativeRoutes(routes){
  ...
  if(!routeAlternatives.length){
    box.innerHTML='<div class="empty">No routes returned by Mappls.</div>';
    return;
  }

  const routeNote=routeAlternatives.length===1
    ?'<div class="hint" style="margin:6px 0 9px;padding:8px;border-radius:8px;border:1px solid #ead9b5;background:#fffaf0">Mappls returned 1 route for this request. The app requested up to 3 routes; Mappls does not guarantee that alternatives are available for every request.</div>'
    :`<div class="hint" style="margin:6px 0 9px;padding:8px;border-radius:8px;border:1px solid #d9e7e1;background:#f7fbf9">Mappls returned ${routeAlternatives.length} routes. All returned routes are shown on the Mappls map.</div>`;

  box.innerHTML=routeNote+routeAlternatives.map((r,i)=>{
    const km=Number(r.distance)/1000;
    const sec=Number(r.duration);
    const tm=Number.isFinite(sec)?formatDurationSeconds(sec):'—';
    return `<div class="history-item" style="cursor:pointer" data-alt-route="${i}">
      <div><b>${i===0?'Primary Route':'Alternative '+i}</b>
      <div class="history-meta">${Number.isFinite(km)?km.toFixed(1)+' KM':'—'} · ${esc(tm||'—')}</div></div>
      <span class="badge">${i===0?'SELECTED':'VIEW'}</span>
    </div>`;
  }).join('');

  box.querySelectorAll('[data-alt-route]').forEach(el=>el.onclick=()=>selectMapplsRoute(Number(el.dataset.altRoute)));
}
...
async function renderMapplsRoute(points,profile,resource){
  ...
  const plotted=drawAllMapplsRoutes(routes);
...
function setDirectionPanelCollapsed(collapsed){
  ...
  if(btn)btn.textContent=directionPanelCollapsed?'☰ Show Route Details':'☰ Route Details';
}
...
async function calculateRoute(){
  ...
  try{
    ...
    lastRoute={km:result.km,time:result.time,legs:result.legs,coords:points.map(p=>[p.lng,p.lat]),geometry:geometryToLatLngs(routes[0]?.geometry||'')};
    renderRouteMetrics(result.km,result.time,'Mappls road distance + routing');
...
function exportMatrixExcel(){
  ...
  try{
    ...
    const summary=[
      ['ETHANOL LOGISTICS — PROFESSIONAL ROUTE PLANNER'],
...
async function showMapView(){
  ...
  // Dashboard / Map View always shows BOTH the map and the current Route Planner.
  ...
  setTimeout(()=>{
    try{
      ...
      else if($('origin').value&&$('destination').value){const a=getLoc($('origin').value),b=getLoc($('destination').value);if(a&&b){mapplsFitBounds([a,b],80);}}
...
for(const t of document.querySelectorAll('.tab'))t.onclick=async()=>{
  ...
  if(t.dataset.tab==='reports')fillDistilleryReportSelect();
  document.querySelectorAll('.topnav button').forEach(x=>x.classList.remove('active'));
...
for(const t of document.querySelectorAll('.topnav button'))t.onclick=async()=>{
  ...
  if(tab==='routeplanner'){showRoutePlannerView();return;}
